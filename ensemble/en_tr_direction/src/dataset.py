...
def get_data(example_cnt):
    ...
    data_train=data_train.rename_column('translation.tr','translation_trg')
    data_val=data_val.rename_column('translation.tr','translation_trg')
    data_train=data_train.rename_column('translation.en','translation_src')
    data_val=data_val.rename_column('translation.en','translation_src')
    #print(type(data))

    return data_train,data_val

def get_modified_data(example_cnt):
    data_train = load_dataset('json',data_files="../train_en_tr.json")['train']
    #data_train=load_dataset('wmt16','tr-en',split='train')
    data_val=load_dataset('wmt16','tr-en',split='validation')
    #data=load_dataset('wmt16','tr-en',split='test')
    #data=data.select(range(example_cnt)) 
...
def get_translation_dataloaders(
    ...
    data_train,data_val=get_modified_data(dataset_size)
...
def get_test_data():
    ...
    data_test=data_test.rename_column('translation.tr','translation_trg')
    data_test=data_test.rename_column('translation.en','translation_src')
    return data_test
...
""" Decided not to use this as I just needed to get test data from here
from tokenizers import Tokenizer

def getTestDataset():
    data_test =get_test_data()
 
    tok_path = "../runs/EN_To_TR_No_Change/tokenizer.json"
